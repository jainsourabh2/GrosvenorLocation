module.exports.getliverpoolOneFF_agg = function(req,res,logger){
...
function getQueryForFootfallData(robj)
{
  ...
  let query1Period=""
  let query1Next = " from (SELECT SUM(CAST(S.FootfallValue as int)) as totalcount,S.StoreName as storename,S.Zone,M.Latitude,M.Longitude,S.YearPeriod,M.Id,M.FloorLevel,S.Period";
  let query1NextPeriod=""

  let query2 = " from (Select * from `dfs`.`default`.`StorefootfallView` V where  V.Period between '"+fromdate+"' and '"+todate+"'";
  let query2Period ="";
  let query3 = " union Select * from `dfs`.`default`.`StorefootfallView` SV where SV.Period between '"+prevyearstartdate+"' and '"+prevyearenddate+"'";
  let query3Period="";
  let query3Next=")";
