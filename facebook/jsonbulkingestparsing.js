...
try{
	...
	for(var d =0; d < parsedJSON.feed.data.length; d++)
	{
		...
		for(var i=0;i<idList.length;i++)
		{
			...
			abusiveflag = false;
			var messageid = (parsedJSON.feed.data[d].id != undefined ? parsedJSON.feed.data[d].id : null);
			output = output + messageid + constants.fieldDelimter;
			
			var messagefrom = (parsedJSON.feed.data[d].from != undefined ? parsedJSON.feed.data[d].from : null);
			output = output + messagefrom.name + constants.fieldDelimter;
